...
typedef struct {
    float *full_logits;       // (max_seq_len * vocab_size)
    int is_kv_filled;         // 标记 KV cache 是否已填充完整序列
} GlobalForwardState;

// 假设 tokens 长度 == max_seq_len
float* llm_forward_full(
    uint32_t *tokens,
    uint32_t max_seq_len,
    LLM *llm,
    LoRA *lora,
    GlobalForwardState *gstate
) {
    LLM_Config *cfg = &llm->config;
    FwdBuffer *s = &llm->state;

    // 阶段1: 预填充 KV cache（只计算 K/V，不计算输出）
    // 注意：此处复用 llm_forward 的前半部分，但跳过后半部分（attention + FFN + logits）
    // 但为最小改动，我们仍调用 llm_forward，但忽略其 logits，并确保 cache 被写入
    for (uint32_t pos = 0; pos < max_seq_len; pos++) {
        // 调用 llm_forward 会写入 k_cache / v_cache at pos
        // 但我们不使用返回的 logits
        (void) llm_forward(tokens[pos], pos, max_seq_len, llm, lora);
    }
    gstate->is_kv_filled = 1;

    // 阶段2: 对每个位置重新运行 forward，但此时 cache 已完整
    // 注意：我们需要“重新计算”每个位置的 Q 和 attention，但 K/V 已在 cache 中
    for (uint32_t pos = 0; pos < max_seq_len; pos++) {
        float *logits = llm_forward(tokens[pos], pos, max_seq_len, llm, lora);
        // 将 logits[pos] 保存到 gstate->full_logits
        memcpy(
            gstate->full_logits + pos * cfg->vocab_size,
            logits,
            cfg->vocab_size * sizeof(float)
        );
    }

    return gstate->full_logits;
}
...
uint32_t generate_next_token(Nano_Context *ctx, uint32_t *output_ids, uint32_t pos, int is_prefilling) {
    ...
    float* logits = llm_forward(output_ids[pos], pos, ctx->max_seq_len, llm, lora);
...
*/
        ...
        // 调用者初始化
        GlobalForwardState gstate;
        gstate.full_logits = (float*)calloc(session->max_seq_len * ctx->sampler->vocab_size, sizeof(float));
        gstate.is_kv_filled = 0;

        for (uint32_t i = 0; i < session->max_seq_len; i++) {
            printf("%d  ", session->output_ids[i]);
        }

        printf("\n");

        // 调用
        float *all_logits = llm_forward_full(session->output_ids, session->max_seq_len, ctx->llm, ctx->lora, &gstate);
        // 使用 all_logits[i*vocab_size + j] 获取位置 i 的第 j 个 logit
        for (uint32_t i = 0; i < session->max_seq_len; i++) {
            float *lg = all_logits + i * ctx->sampler->vocab_size;

            uint32_t next_token = 0;

            // for(uint32_t q = 0; q < ctx->sampler->vocab_size; q++) {
            //     lg[q] /= ctx->sampler->temperature;
            // }

            // softmax(lg, ctx->sampler->vocab_size);

            // flip a (float) coin (this is our source of entropy for sampling)
            // float coin = random_f32(123);

            // if (ctx->sampler->top_p > 0 || ctx->sampler->top_p < 1) {
                // next_token = sample_top_p(lg, ctx->sampler->vocab_size, ctx->sampler->top_p, ctx->sampler->probindex, 114.514);
            // }
            // else {
                // next_token = sample_multinomial(lg, ctx->sampler->vocab_size, 0.114514);
            // }

            next_token = sample_argmax(lg, ctx->sampler->vocab_size);
